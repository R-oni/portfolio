...
let currentGalleryIndex = -1; // Controla qual imagem da galeria está aparecendo
...
// GALERIA DE IMAGENS
// ==========================================
// Para adicionar novas imagens, basta colocá-las nesta lista!
const galleryImages = [
  'mundos/boramu/imagens/sapador.webp',
  'mundos/boramu/imagens/araporu.webp'
  // Exemplo: 'mundos/boramu/imagens/nova_imagem.webp',
];
...
window.initFlipbook = function(wrapperSelector) {
  ...
  // Criamos o slide principal contendo todas as camadas sobrepostas
  ...
  // Gera automaticamente as tags <img> para todas as imagens da array
  ...
  // Lógica de Sequência Inicial (Tempo guardado na array introTimeouts)
  ...
  handleTouchEnd = function(e) {
    ...
    if (diff > 50) { 
      avancarCena();  // Swipe para cima = Avançar
    } else if (diff < -50) {
      voltarCena();   // Swipe para baixo = Voltar
    }
  };
  ...
  handleWheel = function(e) {
    if (!swipeEnabled) return;
    if (e.deltaY > 0) { 
      avancarCena();  // Rolar para baixo = Avançar
    } else if (e.deltaY < 0) {
      voltarCena();   // Rolar para cima = Voltar
    }
  };
  ...
  function avancarCena() {
    ...
    if (appState === 'system') {
      ...
      if (galleryImages.length === 0) {
        swipeEnabled = true; debounce = false; return;
      }
  ...
  function voltarCena() {
    ...
    if (appState === 'system') {
      // Bloqueia retrocesso antes do sistema
      swipeEnabled = true;
      debounce = false;
      ...
      setTimeout(() => {
        const layerSystem = document.getElementById('layer-system');
        // Ao voltar, faz um efeito reverso (zoom out) do scale(4) para o scale(1)
        layerSystem.style.transition = 'opacity 1.5s ease-out, transform 1.5s cubic-bezier(0.16, 1, 0.3, 1)';
        layerSystem.style.transform = 'scale(1)';
        layerSystem.style.opacity = '1';
      ...
      if (currentGalleryIndex > 0) {
        // Volta para a imagem anterior na galeria
        const currentImg = document.getElementById(`layer-gallery-${currentGalleryIndex}`);
        ...
        // Volta da primeira imagem para o globo
        ...
        setTimeout(() => {
          ...
          swipeEnabled = true;
          debounce = false;
        }, 600); // 0.6s da transição da imagem apagando
      }
    }
  }
};
